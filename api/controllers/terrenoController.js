const Terreno = require('../models/terrenoSchema');
...
exports.adicionar = async (req,res) => {
    try{
        ...
        const result = await Terreno.create(req.body)
...
exports.listarTudo = async (req,res) => {
    try{
        const result = await Terreno.find({})
...
exports.buscaPorId = async (req,res) => {
    try{
        ...
        const result = await Terreno.findById({_id:req.params.id})
        return res.status(200).json(result)
    } catch(err){
        console.error(err.message)
        return res.status(400).json({message:"ERRO"})
...
exports.apagar = async (req,res) => {
    try{
        ...
        await Terreno.findByIdAndDelete({_id:req.params.id})
...
exports.atualizar = async (req,res) => {
    try{
        ...
        const result = await Terreno.findByIdAndUpdate({_id:req.params.id}, req.body)
